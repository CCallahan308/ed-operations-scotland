# ED Operations Analytics â€” NHS Scotland A&E Compliance Forecasting
...
>
> **Status:** Phases 0â€“7 complete, plus a post-audit hardening pass. 111 tests (37 run on the committed fixture; the rest need the dataset). Holdout evaluated once.

## Headline result (honest)

Candidate A is a gradient-boosted-tree + persistence ensemble. On the held-out 12 months (2025-06 â†’ 2026-05):

| Model | Holdout MAE | 95% CI | Notes |
|---|---|---|---|
| **Candidate A** | **2.72 pp** | [2.50, 2.95] | beats persistence on point estimate |
| Persistence baseline | 2.87 pp | â€” | the bar |
| Seasonal naive | 4.12 pp | â€” | biased high by structural break |

**Candidate A beats persistence by +0.15 pp on the holdout, but the paired-bootstrap 95% CI on the improvement [-0.006, +0.299] includes zero â€” the gain is directionally real but not statistically significant on a 12-month evaluation window.** This is reported without hedging; see [`docs/HOLDOUT_PHASE6.md`](docs/HOLDOUT_PHASE6.md).

## What this is

A portfolio flagship blending Business Intelligence, Business Analytics, and Data Science on **real Public Health Scotland A&E open data** (site-month grain, 2007-07 â†’ 2026-05, 103 treatment locations, 4h/8h/12h breach statistics).
...
- Source: [Monthly A&E Activity and Waiting Times â€” Public Health Scotland](https://www.opendata.nhs.scot/dataset/monthly-accident-and-emergency-activity-and-waiting-times)
...
â”œâ”€â”€ app.py              # Streamlit dashboard (thin view over committed artifacts)
â”œâ”€â”€ .github/workflows/  # ci.yml (ruff + pytest on push/PR, Python 3.11/3.12)
â”œâ”€â”€ .streamlit/         # theme config for deployment
â”œâ”€â”€ data/
â”‚   â”œâ”€â”€ raw/            # PHS exports (gitignored; fetch via scripts/fetch_data.py)
â”‚   â”œâ”€â”€ external/       # reserved for holidays/weather (not used in v1)
â”‚   â””â”€â”€ processed/      # rebuilt artifacts (gitignored; reproducible)
â”œâ”€â”€ docs/               # phase deliverables, BI data model, review handoff, execution log
â”œâ”€â”€ notebooks/          # 01_data_exploration.ipynb (reproducible EDA)
â”œâ”€â”€ pipeline/           # score_holdout.py (one-shot holdout scoring)
â”œâ”€â”€ reports/            # frozen config, metrics, feature importance, dashboard_data.json
â”œâ”€â”€ scripts/            # fetch_data.py, run_sql.py, build_dashboard_data.py
â”œâ”€â”€ sql/                # DuckDB analytics layer (fact + dims + aggregations + validations)
â”œâ”€â”€ src/ed_ops/         # config, data_quality, splits, baselines, features, model, evaluation, dashboard_data
â””â”€â”€ tests/              # 111 tests across 10 modules (fixture-runnable; full-data tests skip)
```
...
feature importance, limitations). No fabricated KPIs â€” every figure is computed from
...
- **Leakage audit at every phase** â€” 111 tests enforce raw-data integrity, count identities, no-target-in-features (L1), all-lags-â‰¤-t (L2), chronological split (L3), and no-holdout-in-training (L5).
- **Honest baselines** â€” three baselines including seasonal naive; the actual bar turned out to be persistence (validation MAE 2.85 pp; 2.87 pp on the holdout), not seasonal naive as originally framed.
- **Honest reporting of a negative-leaning result** â€” when Candidate A's tree alone lost to persistence (Phase 5), it was reported, not tuned away. When the holdout improvement failed to reach statistical significance (Phase 6), the CI was reported, not hidden.
- **Documented structural break** â€” NHS Scotland A&E compliance fell from ~97% (2007) to ~67% (2026) and is still declining. The model is evaluated on its ability to forecast *through* this regime change, which is the honest problem.

## Key documentation

| Doc | Purpose |
|---|---|
| [`docs/REVIEW_HANDOFF.md`](docs/REVIEW_HANDOFF.md) | **Review board handoff** â€” what to challenge, what's defensible, success criteria. *Start here for external review.* |
| [`docs/EXECUTION_LOG.md`](docs/EXECUTION_LOG.md) | Authoritative execution record (Phases 0â€“7, decisions D001â€“D021) |
| [`docs/PROBLEM_FRAMING.md`](docs/PROBLEM_FRAMING.md) | Phase 1: decision, target, metrics, leakage surface, non-claims |
| [`docs/DATA_QUALITY.md`](docs/DATA_QUALITY.md) | Phase 2: findings F001â€“F005, cleaning rules |
...
6. No causal claims â€” the model predicts; it does not estimate intervention effects.
