...
|---|---|---|
| Full test suite | `python -m pytest tests/ -v` | **85 passed** across 6 modules |
...
| Frozen config integrity | compare current `train_candidate_a()` to `reports/candidate_a_config.json` | Exact match (weight 0.4, val MAE 2.5091) |
...
2. **`requirements.txt` was incomplete.** Phase 0 note said modeling deps would be added in Phase 4/5 — that never happened. Code imports `sklearn` (Phase 5) and `matplotlib` (Phase 5/6 figures) but neither was declared. Fixed: added `scikit-learn>=1.4` and `matplotlib>=3.8`; removed unused `black` (we use `ruff format`). Verified: third-party imports now exactly match declared deps (`numpy, pandas, pytest, sklearn` in tested code; `matplotlib` in analysis scripts).
...
| Validation MAE 2.509 | MODEL_PHASE5.md / candidate_a_config.json | 2.5091 ✓ |
| Ensemble weight 0.4 | MODEL_PHASE5.md / candidate_a_config.json | 0.4 ✓ |
| Holdout MAE 2.723 | HOLDOUT_PHASE6.md / holdout_evaluation.json | 2.7226 ✓ |
| Holdout 95% CI [2.488, 2.941] | HOLDOUT_PHASE6.md | [2.4879, 2.9405] ✓ |
...
- The frozen Candidate A model (val MAE 2.5091) via `train_candidate_a()`.
- The holdout evaluation (MAE 2.7226) via `pipeline/score_holdout.py`.
...
- **Tests:** `tests/` (6 modules, 85 tests, all green)
